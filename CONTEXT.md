...
- **2026-09-18**: ✅ **Drive FUNCIONA de punta a punta.** Un comprobante real subio a `PJ04 VIATICOS`, quedo visible con el enlace, la URL se guardo en la fila, y el archivo de prueba se borro. **Sin tocar una linea de `apps-script.gs`.**
  ...
  - **Drive usa OAuth de usuario; las hojas siguen con la cuenta de servicio.** Son dos credenciales distintas a proposito: en las hojas no hay que **crear** archivos, solo leer y escribir en uno que ya existe.
  - Actua como **`contabilidad@energy-millennium.com`** — cuenta institucional, no personal: los comprobantes no quedan atados a la cuenta de una persona.
  - 🛑 **Se evito un incidente grave.** La pantalla de OAuth ofrecia **"Make internal"**, que suena mas seguro. Pero **esa misma pantalla controla el login de la app**: pasarla a Internal habria dejado afuera a **7 de los 9 usuarios** (5 de `energy-millennium.com` y 2 de Gmail; solo 2 son de `ylevigroup.com`), sin aviso y sin forma de revertirlo desde la app. **Se verifico leyendo la hoja USUARIOS antes de tocar nada.** Debe quedar en **External / In production**.
    - *In production* es ademas lo que hace que la autorizacion **no venza**: en modo *Testing* vencerian a los 7 dias.
  - Cliente OAuth **aparte** del login (`pj04-backend-drive`, tipo *Desktop app*): si algun dia hay que cortarle el acceso a Drive al servidor, se revoca ese y **el ingreso de los 9 usuarios sigue intacto**.
  - `autorizar-drive.js` se corre **una sola vez** y guarda `token-drive.json`. `.gitignore` cubre `oauth-*.json` y `token-*.json`.
...
- **2026-09-18**: ✅ **Paso 2.1 — la logica contable real YA CORRE sobre el servidor propio.** `apps-script.gs` se carga **sin modificar** sobre un entorno de Node y calcula lo mismo. **57 comprobaciones nuevas**, en `servidor/` (`npm run prueba`).
  ...
  - **`fechas-sheets.js`** — ⚠️ **hallazgo importante:** la API devuelve las fechas como **numero de serie**, y `apps-script.gs` pregunta `valor instanceof Date` en todos lados. Peor: **un serial de 46000 y un pago de 46.000 pesos son el mismo numero**, no hay forma de distinguirlos mirando el valor. Por eso se convierten **solo las columnas que el codigo declara como de fecha**, por nombre. Adivinar convertiria importes en fechas sin que nada fallara.
    - Se descarto pedirle a la API el texto ya formateado: **depende del idioma del documento**, que es de donde salio el problema de dia/mes.
  - **`entorno.js`** — `Utilities` (los 7 patrones de `formatDate` que usa el codigo, y **solo esos**: cualquier otro falla en voz alta), `CacheService`, `Logger`, `Session`, `ContentService`. `formatDate` usa `Intl` con la zona pedida: **EasyPanel corre en UTC**, asi que usar la hora del proceso correria todas las fechas cinco horas.
    - `computeDigest` devuelve bytes **con signo** como Apps Script. Devolverlos sin signo cambiaria todos los hashes y **dejaria afuera a todas las sesiones abiertas**.
  - 🚧 **BLOQUEO ENCONTRADO:** registrar un pago llama a **`DriveApp` SIEMPRE**, aunque no haya archivos adjuntos, porque resuelve la carpeta de la seccion antes de mirar si hay algo que subir. **Registrar pagos no se puede migrar hasta implementar Drive.** Hay una comprobacion que lo fija para que no se descubra tarde. Consultas, saldos y conciliacion **si** funcionan ya.
...
- **2026-09-18**: 🏗️ **MIGRACION EN CURSO — backend propio en EasyPanel (opcion A, elegida por el usuario).** Las hojas **siguen siendo la fuente de datos**; lo que cambia es quien las lee. Ver [servidor/README.md](servidor/README.md).
  ...
  - **Plan por pasos, en orden:**
    1. ✅ **Andamiaje + medicion — HECHO, resultado decisivo (2026-09-18).** Las **12 pestanas** (169 filas, 1.992 celdas) en **UNA llamada HTTP**: **598 ms** de promedio (637 / 635 / 561 — estable, no erratico). Apps Script costaba **2.085 ms por UNA sola hoja** y hasta 33.810 ms en el peor caso. Ese medio segundo es casi todo **costo fijo de la API**, no volumen: con diez veces mas registros tardara practicamente lo mismo. Medido desde la oficina; desde EasyPanel deberia bajar. **Cuenta de servicio:** `pj04-backend@pj04-control-de-pagos.iam.gserviceaccount.com`, en el proyecto `pj04-control-de-pagos` — el **mismo** donde ya vivia el ID de cliente del login (`165996240052-…`), asi que todo queda junto.
    1b. ⚠️ Al renombrar la clave, Windows dejo `credenciales.json.json` (oculta las extensiones). El patron `credenciales*.json` del `.gitignore` la cubria igual. Verificado con `git status` que git no la ve. `servidor/src/hojas.js` (acceso por `values.batchGet`: TODAS las hojas en UNA llamada HTTP) y `servidor/src/medir.js` (solo lee, no escribe). **Decide si seguimos**: si la mejora no es grande, no se migra.
    2. **Infraestructura primero** (estrategia elegida por el usuario, 2026-09-18). Se implementa en Node la superficie de Apps Script y **`apps-script.gs` corre TAL CUAL encima**. La logica contable probada **no se toca**, asi que no puede haber regresiones de negocio, y las 4 suites siguen valiendo. La limpieza en modulos propios queda para despues, de a poco y con las pruebas cuidando cada movimiento.
       - 🔑 **El banco de pruebas YA define el contrato**: las 4 suites llevan meses implementando un `SpreadsheetApp` falso (`hojaFalsa`, `getRange`, `appendRow`, `setValues`, `deleteRow`...). El adaptador es **ese mismo**, pero contra la API real. Eso reduce muchisimo el riesgo de olvidar un metodo.
       - **Superficie a implementar** (medida sobre el archivo): `Utilities.formatDate` (22 usos), `SpreadsheetApp.getActiveSpreadsheet` (16), `Logger.log` (8), `MailApp.sendEmail` (6), `DriveApp.getFileById` (5), `CacheService.getScriptCache` (5), `UrlFetchApp.fetch` (3), `Utilities.base64*` (3), `getUuid` (2), `computeDigest` (2), `SpreadsheetApp.flush` (2), `ScriptApp.getOAuthToken` (2), `DriveApp.getFoldersByName` (2), mas `newBlob`, `SpreadsheetApp.create`, `Session.getScriptTimeZone`, `DriveApp.createFolder/Permission/Access`, `ContentService`.
       - **Lo unico que no es trivial:** `DriveApp` (subida de comprobantes) va contra la **Drive API**, y `MailApp.sendEmail` necesita un emisor real (SMTP o Gmail API). Todo lo demas es directo.
...
- **2026-09-17**: 🐛 **REGRESIÓN PROPIA, corregida: los traslados del mismo día no sumaban.** El usuario trasladó 100.000 de Millennium a Viáticos y el fondo no subió. Se reprodujo y se verificó contra el código anterior: **el traslado se ignoraba en los DOS lados** — ni restaba del banco ni sumaba al fondo.
  ...
  - **`momentoDeTraslado_`**, de más preciso a menos: si se registró el mismo día en que se hizo, vale el **instante del registro** (tiene hora exacta); si es un traslado viejo cargado después, vale el **final de ese día**. La primera regla es la que evita **contar dos veces** cuando el saldo real se carga *después* de registrar el traslado — hay una comprobación dedicada a ese caso, sin la cual la mutación correspondiente no se detectaba.
  - Confirmado también que **Viáticos y Compra Materiales comparten bolsa**: hay una prueba con el escenario completo (base + traslado que suma + viático que resta + materiales que resta).
  - **11 comprobaciones nuevas y 6 mutaciones verificadas.** `REVISION_BACKEND` -> `2026-09-17-e`.
...
- **2026-09-16**: 🛠️ **`PASO_1` / `PASO_2` / `PASO_3`: las tres funciones que se corren con un clic.** El boton "Ejecutar" del editor de Apps Script **no puede pasar parametros**, asi que instrucciones como `repararFechasFuturas(false, true)` o `normalizarFechasRegistro(false)` eran **imposibles de ejecutar desde el menu**. Se le estuvo pidiendo al usuario algo que no se podia hacer.
  ...
  - 🧭 **Leccion:** verificar que las instrucciones que se le dan al usuario sean **ejecutables en la interfaz que el usuario tiene delante**, no solo correctas en el codigo. `REVISION_BACKEND` -> `2026-09-16-i`.

- **2026-09-16**: 🧹 **Las 8 sospechosas confirmadas, y `repararFechasFuturas(simular, incluirSospechosas)`.** El detector encontro **8** fechas mal convertidas que NO estaban en el futuro y por eso escapaban a la regla. La evidencia resulto concluyente: **6 de 8 caen exactamente sobre su fecha de pago** al intercambiar dia y mes (filas 2, 3, 30, 31, 32, 33 de PAGOS REGISTRADOS) y las otras 2 a pocos dias. Ademas son contiguas a las filas 6-9, que si se corrigen por la regla del futuro: **el mismo bloque migrado, el mismo formato mes/dia**.
  - Con la lectura equivocada esos registros quedarian escritos **meses ANTES** de pagar. Que eso pase en 8 de 8, y que en 8 de 8 el intercambio caiga sobre el pago, no es casualidad.
  - **Corregirlas hay que pedirlo a proposito** (`repararFechasFuturas(false, true)`). Es una decision distinta de la automatica: ahi las dos lecturas caen en el pasado, la regla "no puede ser futuro" no decide, y lo que decide es la coherencia con el pago — que es un indicio fuerte, no una prueba. Por eso no se corrige sola.
  - Total en la hoja: **27 por la regla del futuro + 8 por coherencia con el pago = 35 de 40** fechas reales mal convertidas. **7 comprobaciones nuevas, 3 mutaciones verificadas.** `REVISION_BACKEND` -> `2026-09-16-h`.

- **2026-09-16**: 🧪 **Confirmacion independiente de la reparacion, y el cabo suelto que faltaba.** El simulacro de `repararFechasFuturas` corrigio **27 de 40** fechas reales revisadas. Lo que lo vuelve seguro no es la regla en si, sino que **`FECHA DE PAGO` —que no participo en la conversion ni se toca— corrobora cada correccion**: Viaticos fila 5 queda en `2026-09-11` con pago `2026-09-11`; filas 17-22 en `2026-09-12` con pago `2026-09-12`; Caja Menor 3-5 en `2026-09-10` con pago `2026-09-10`. Con la lectura equivocada el registro caia **meses despues** del pago, lo que es imposible.
  - **El cabo suelto:** las 13 que no estaban en el futuro escapan a la regla, pero pueden estar igual de mal. Una fila que era `08/05/2026` (5 de agosto en mes/dia) se convirtio en **8 de mayo** — tambien pasado, tambien invisible.
  - **Como se detectan:** por la distancia a su FECHA DE PAGO. Un registro se escribe cerca del pago, no meses despues. Si la fila esta a mas de 45 dias de su pago y al intercambiar dia y mes cae a menos de 7, se marca con `??`.
  - ⚠️ **Esas NO se corrigen solas**, a diferencia de las futuras: aca las dos lecturas son posibles (ambas caen en el pasado) y la regla "no puede ser futuro" no decide. Solo quien conoce la operacion puede. **4 comprobaciones nuevas, 2 mutaciones verificadas.** `REVISION_BACKEND` -> `2026-09-16-g`.

- **2026-09-16**: ✅ **ENCONTRADA la causa del desorden: 40 fechas de registro guardadas en el FUTURO.** El diagnostico lo mostro sin lugar a dudas — los 10 registros mas recientes segun el servidor estaban todos en **diciembre de 2026**, y todos eran **fechas reales**, no texto.
  - **Que paso.** Esas 40 filas migradas tenian FECHA REGISTRO en texto **mes/dia** (`09/12/2026` = 12 de septiembre). Al convertir la columna a fecha real, Sheets las leyo como **dia/mes** y las guardo como **9 de diciembre**. Como una fecha de registro futura ordena primero, taparon los pagos recientes de verdad. La evidencia que lo confirma: las 6 de Viaticos en `2026-12-09` tienen horas 10:05, 10:11, 10:16, 10:23, 12:52, 14:31 — la secuencia de una jornada, que leida como mes/dia es el 12 de septiembre.
  - **`repararFechasFuturas(simular)`**: corrige intercambiando dia y mes. Esto **si** se puede automatizar, a diferencia del texto ambiguo, porque una fecha de REGISTRO no puede ser del futuro: la escribe el sistema al registrar. Si el intercambio da una fecha pasada, es la unica lectura posible. Respalda antes de tocar y exige simulacro.
  - ⚠️ **`FECHA DE PAGO` NO se toca**: un pago **si** puede estar fechado en el futuro, asi que ahi la regla no aplica. Se listan sus valores en las filas corregidas para revisarlas a ojo.
  - **11 comprobaciones nuevas** y **5 mutaciones** verificadas. Para lograrlo hubo que arreglar el banco de pruebas: `hojaFalsa` no tenia `setValue`, asi que **ninguna funcion que escribe era comprobable** — justo las mas caras si fallan. Ahora `backend(hojasExtra)` acepta hojas a medida.
  - 🧭 **Leccion de todo el episodio:** se perdieron varias rondas depurando el *sintoma* (el orden) y dos hipotesis que encajaban con el sintoma pero eran falsas (la heuristica dia/mes, y las zonas horarias). Lo que lo resolvio fue **listar los datos crudos ordenados como los ve el servidor**. Cuando algo "sigue mal", mostrar los datos antes que teorizar sobre ellos. `REVISION_BACKEND` -> `2026-09-16-f`.

- **2026-09-16**: 🔍 **Correccion de un diagnostico propio, y zonas horarias como sospechoso real.** El simulacro de `normalizarFechasRegistro` sobre los datos reales mostro que **las 68 fechas de texto tienen todas el primer numero > 12** (13, 14, 15, 18, 21, 25...). O sea que **ninguna era ambigua** y ni el codigo viejo ni el nuevo las leian mal. Y `FECHA DE PAGO` resulto estar **ya en fechas reales**: lo que se veia distinto en la hoja (`9/15/2026` vs `2026-09-15`) era **formato de visualizacion**, no datos mezclados.
  - ⚠️ **O sea que la explicacion "el backend confunde dia y mes" era falsa**, aunque encajaba con el sintoma. Encajar con el sintoma no es evidencia. El arreglo de formato-por-columna sigue siendo correcto como proteccion, y escribir fechas reales sigue siendo la causa raiz de los "Invalid" de la Tabla — pero no eran la causa del desorden.
  - **Nuevo sospechoso, con mecanismo concreto:** una celda que es fecha REAL guarda un instante y `Utilities.formatDate(v, ZONA_HORARIA, ...)` la **convierte de zona**; una celda de TEXTO se arma con `new Date(a, m, d, hh, mm)`, que usa la zona del **script**. Si la zona de la hoja, la del script y `ZONA_HORARIA` no son la misma, **los dos grupos quedan corridos en horas entre si** — cada registro creible por separado, el orden equivocado al intercalarlos. Con 148 celdas reales y 68 de texto, es exactamente la forma del sintoma.
  - `revisarFechasRegistro()` ahora informa las **tres zonas** y el mismo instante formateado en cada una, que es la comparacion que importa (los nombres pueden diferir sin que haya desfase).
  - 🛑 **No normalizar hasta confirmar que las tres coinciden**: normalizar con las zonas desalineadas grabaria el corrimiento en la hoja, y ahi ya no se puede distinguir del dato. `REVISION_BACKEND` -> `2026-09-16-e`.
...
- **2026-09-16**: 🔬 **Dos diagnosticos para cerrar el problema de fechas.** El sintoma ("sigue mal") no distinguia entre *el backend no convierte* y *el navegador corre una copia vieja*, asi que se hizo visible cada uno por separado:
  ...
  - **`probar_fecha` confirmo que el backend SI esta al dia** (revision correcta) y que la heuristica hace lo que fue disenada para hacer. El defecto esta en el diseno, no en el despliegue: **decide cada celda por separado**. Si la hoja escribe dia/mes de forma consistente, un `09/12/2026` (9 de diciembre) cae en la regla de "no puede ser futuro" y se convierte en 12 de septiembre — fecha equivocada y orden equivocado, sin que nada falle.
  - **`revisarFechasRegistro()` reescrito para que decida con evidencia** (corre desde el editor, **no necesita redespliegue**): cuenta cuantos registros *solo pueden* ser dia/mes (primer numero > 12) contra cuantos *solo pueden* ser mes/dia, agrupa los valores crudos por **forma** (digitos -> `9`) para que cualquier formato inesperado salte a la vista, y lista los 10 mas recientes segun los ordena hoy el servidor, con el valor crudo al lado. Eso separa "la hoja se lee mal" de "la pantalla los muestra mal".
