...
- **Cada vez que se modifique `index.html`** (o cualquier asset cacheado), sube el número de versión de `CACHE` en `sw.js` (ej. `control-pagos-v9` → `v10`). Sin esto, la PWA sigue mostrando la versión vieja en los dispositivos de los usuarios porque el Service Worker es cache-first.
